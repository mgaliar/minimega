...
version the `spec` section conforms to (right now there's only `v1` for all
...
A topology is comprised of one or more _nodes_ (a VM or container), each
...
`phenix` apps ([default]() or [user]()) for use on a topology. In this sense, a
...
which can only be applied to an accompanying topology named `foobar` (note that
the topology and scenario names do not have to match). Included in this scenario
is an experiment app named `miniccc-injector` and three host apps: `startup`,
`protonuke` and `wireguard`. Each entry in the list of app hosts includes custom
app metadata and the hostname of the topology node to apply the metadata to.
